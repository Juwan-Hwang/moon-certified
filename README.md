...
**v0.8.0** — 70+ 个算法包，`moon check` 0 warnings 0 errors，`moon test` 1091 tests 全部通过，`moon prove` 9/9 包验证通过 (0 失败)。

### v0.8.0 变更亮点（新增算法包 + 质量提升）

- **新增 8 个算法包**：Euler 线性筛（数论，O(n) 筛法 + O(log n) 因式分解）；Sparse Table RMQ（泛型，O(1) 幂等查询）；LCA 最近公共祖先（图论，二进制提升 O(n log n) 预处理）；线段相交+点在多边形内（计算几何，精确整数叉积）；Dinic 最大流（图论，Dinic 算法）；最近点对（计算几何，分治 O(n log n)）；矩阵运算（数学，高斯消元 + 矩阵乘法）；组合数学（数学，组合数/Catalan/Stirling 数，Int64 溢出保护）
- **消除全部编译器警告**：combinatorics 测试中未使用变量修复，`not()` 替换为 `!`
- **abort() 审查**：库代码中唯一 abort 位于 RB 树 `del_min`（泛型 T 无法返回默认值，且该分支不可达），其余 abort 均在测试代码中
- **测试覆盖**：新增 68 个测试（总计 1091），全部通过
