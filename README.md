...
**v0.1.0** — 25 个算法包，299 个测试全部通过，`moon prove` 9/9 包验证通过 (0 失败)。
...
| segment_tree | 12 | 区间和查询 + 单点更新 |
| bfs_dfs | 33 | 含输入校验、早退路径、自环 |
| topological_sort | 12 | Kahn 算法，含环检测 |
| kruskal | 14 | 最小生成树，复用 union_find |
| bellman_ford | 9 | 负权最短路，含负环检测 |
| floyd_warshall | 8 | 全源最短路，含负环检测 |
| dijkstra | 9 | 含输入校验 |
| kmp | 17 | 共享 kmp_step helper，含重叠匹配 |
| sieve | 10 | 素数筛 + 素数判定 |
| ext_gcd | 8 | 扩展欧几里得 + 模逆元 |
| lcs | 7 | 最长公共子序列 |
| edit_distance | 6 | 编辑距离 |
| knapsack | 7 | 0/1 背包 |
...
│   └── segment_tree/      🔒 线段树 (tested, 12 tests)
├── graph/
...
│   └── advanced/           🔒 Bellman-Ford + Floyd-Warshall (tested, 17 tests)
├── string/
│   └── kmp/                🔒 KMP (tested, 17 tests)
├── number_theory/
│   ├── gcd/                ✅ GCD (verified)
│   ├── fast_power/         ✅ 快速幂 (verified)
│   └── prime/              🔒 素数筛 + 扩展欧几里得 (tested, 18 tests)
├── math/
│   └── array_sum/          ✅ 数组求和 (verified)
├── dp/
│   └── dp/                 🔒 LCS + 编辑距离 + 背包 (tested, 20 tests)
├── .github/workflows/
...
| kruskal | 14 | 🔒 tested |
| bellman_ford + floyd_warshall | 17 | 🔒 tested |
| dijkstra | 9 | ✅ verified |
| kmp | 17 | 🔒 tested |
| gcd | 5 | ✅ verified |
| fast_power | 5 | ✅ verified |
| sieve + ext_gcd | 18 | 🔒 tested |
| array_sum | 6 | ✅ verified |
| lcs + edit_distance + knapsack | 20 | 🔒 tested |
| **Total** | **299** | **9 verified, 16 tested** |
