...
**v0.1.0** — 19 个算法包，200 个测试全部通过，`moon prove` 9/9 包验证通过 (0 失败)。
...
| 包 | 测试数 | 说明 |
|---|--------|------|
| insertion_sort | 10 | 循环边界验证；排序性仅测试 (数组变更导致 Z3 丢失量化不变量) |
| selection_sort | 8 | 循环边界验证；排序性仅测试 (同上) |
| merge_sort | 12 | 递归排序 (调用非 contracted 辅助函数) |
| quick_sort | 11 | 快速排序 (同上) |
| binary_heap | 15 | 堆操作 (含输入校验、容量检查、置换测试) |
| bfs_dfs | 33 | 图遍历 (含输入校验、早退路径、自环) |
| kmp | 18 | 字符串匹配 (含重叠匹配、LPS 验证) |
...
| bst | 19 | 🔒 tested |
| **Total** | **200** | **9 verified, 10 tested** |
