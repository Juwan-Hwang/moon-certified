...
**v0.1.0** — 16 个算法包，130 个测试全部通过，`moon prove` 9/9 包验证通过，65 个证明目标全部由 Z3 自动证明 (100%)。
...
| dijkstra | 57 goals | 数组边界 + 结果长度 (for 累加器 + graph_index_bound 引理) |
...
2. **proof_axiomatized 引理**：对 Z3 无法自动证明的非线性算术事实（如 `u*n+v < len` 从 `u<n, v<n, n*n<=len`），声明为 `proof_axiomatized` 引理，使验证器直接采信
3. **while 转 for**：BFS/DFS 的队列循环改为有界 for 循环，使终止性可证
...
│   └── dijkstra/           ✅ Dijkstra (verified, 57 goals)
├── string/
...
| bfs_dfs | 14 | 🔒 tested |
| dijkstra | 10 | ✅ 57 goals |
| kmp | 15 | 🔒 tested |
| gcd | 5 | ✅ 1 goal |
| fast_power | 5 | ✅ 1 goal |
| array_sum | 6 | ✅ 2 goals |
| **Total** | **130** | **65 proved, 0 timeout (100%)** |
