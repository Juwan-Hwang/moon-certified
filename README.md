...
**v0.1.0** — 16 个算法包，130 个测试全部通过，8 个包通过 `moon prove` 形式化验证（55 个证明目标全部 discharged by Z3）。
...
| 包 | 已证/总数 | 说明 |
|---|----------|------|
| insertion_sort | 2/4 goals | 内层循环移位操作复杂度高 |
| selection_sort | 13/22 goals | 分区不变量复杂 |
| bfs_dfs | 4/5 goals | 仅终止性超时 |
| dijkstra | 27/45 goals | 松弛操作证明复杂 |
...
├── sorting/
│   ├── insertion_sort/     ⚠️ 插入排序 (partial)
│   ├── selection_sort/     ⚠️ 选择排序 (partial)
...
├── graph/
│   ├── bfs_dfs/            ⚠️ BFS/DFS (partial)
│   └── dijkstra/           ⚠️ Dijkstra (partial)
├── string/
│   └── kmp/                🔒 KMP (tested)
├── number_theory/
│   ├── gcd/                ✅ GCD (verified)
...
| insertion_sort | 10 | ⚠️ 2/4 goals |
| selection_sort | 8 | ⚠️ 13/22 goals |
| merge_sort | 12 | 🔒 tested |
| quick_sort | 11 | 🔒 tested |
| is_sorted | 5 | ✅ 1 goal |
| binary_heap | 11 | 🔒 tested |
| bfs_dfs | 14 | ⚠️ 4/5 goals |
| dijkstra | 10 | ⚠️ 27/45 goals |
| kmp | 15 | 🔒 tested |
| gcd | 5 | ✅ 1 goal |
| fast_power | 5 | ✅ 1 goal |
| array_sum | 6 | ✅ 2 goals |
| **Total** | **130** | **55 proved, 30 timeout** |
